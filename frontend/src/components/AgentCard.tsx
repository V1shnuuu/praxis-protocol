  ...
  return (
    ...
    >
      <header className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="truncate text-sm font-semibold text-ink-900">{agent.name}</h3>
          ...
          <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-ink-500">
            ...
            <Mono className="text-ink-500">#{agent.agentId}</Mono>
            <span aria-hidden>·</span>
            <Mono className="text-ink-500" title={agent.owner}>
      ...
      <div>
        ...
        >
          <span className="mt-0.5 shrink-0 text-[10px] uppercase tracking-wider text-ink-500">Policy</span>
          <span
            className={`flex-1 text-xs leading-relaxed text-ink-600 ${policyOpen ? "" : "line-clamp-2"}`}
          >
            {agent.policy}
          </span>
        </button>
      ...
      <dl className="grid grid-cols-3 gap-3 border-t border-line-200 pt-3">
        ...
        <div>
          <dt className="text-[10px] uppercase tracking-wider text-ink-500">Attestations</dt>
          <dd className="mt-1 text-sm font-medium tabular-nums text-ink-900">
        ...
        <div>
          <dt className="text-[10px] uppercase tracking-wider text-ink-500">Slashes</dt>
          <dd
            className={`mt-1 text-sm font-medium tabular-nums ${
              agent.slashCount > 0 ? "text-state-slashed" : "text-ink-900"
            }`}
          >
            {agent.slashCount}
          </dd>
